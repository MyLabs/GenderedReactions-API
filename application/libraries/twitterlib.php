...
// 					$query=implode('+OR+',$this->terms);
					$query = $term;
					$query_data = array('q' => $query, /* 'result_type' => 'recent',  'include_entities' => 'true',*/ 'rpp' => '100',/*'result_type'=>'mixed',*/ 'count'=>'100', 'lang'=>'en');
	...
	*/
	function save($data=null,$result=false)
	{
		...
		if ( (is_array($data) && isset($data['id_str']) ) || ( is_object($data) && isset($data->id_str) ) ) {
			if ( is_array($data) && isset($data['from_user_id_str']) ){ // if we are dealing with search api
				$user_id=$data['from_user_id_str'];
				$tweet_id=$data['id_str'];
				$tweet = $data['text'];
				$created_at = $data['created_at'];
				$location = $data['user']['location'];
				$lang = $data['metadata']['iso_language_code'];
			}
			...
			{
				// set input
				$input=array( 'tweet_id' =>  $tweet_id, 'user_id' => $user_id, 'tweet' => $tweet, 'created_at' => $created_at, 'location' => $location, 'lang' => $lang);
				// save tweet in db
