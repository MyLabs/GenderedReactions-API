...
class Fda extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('side_effects_model');
		$this->load->model('side_effects_lookup_model');
	}
...
// 				$test = mysql_query("INSERT INTO side_effects VALUES ('', '$sideEffectName', '')");
				...
				$test = $this->side_effects_model->insert($arr);
...
// 				mysql_query("INSERT INTO side_effects_lookup VALUES ('', $drugId, $sideEffectId, '', '', '')");
				...
				$test = $this->side_effects_lookup_model->insert($arr);
				return mysql_insert_id();
			}
			else {
				$row = mysql_fetch_array($result);
...
// 				$test = $this->db->query($query);
				...
				$test = $this->drugs_model->insert($arr);
				echo 'inserting drug:';
				return mysql_insert_id();
			}
			else {
